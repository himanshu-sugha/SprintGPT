...
resolver.define('compareSprints', async (req) => {
    return exports.compareSprintsHandler({ payload: req.payload });
});

resolver.define('generateRetroReport', async (req) => {
    return exports.generateRetroHandler({ payload: req.payload });
});

/**
 * Create a new Confluence page with the sprint retrospective report
 */
resolver.define('createConfluencePage', async (req) => {
    console.log('=== CREATE CONFLUENCE PAGE RESOLVER ===');
    ...
    try {
        ...
        if (!spacesResponse.ok) {
            console.log('Failed to get spaces:', spacesResponse.status);
            return {
                success: false,
                message: 'Cannot access Confluence spaces. Please check permissions.'
            };
        }

        const spacesData = await spacesResponse.json();
        console.log('Spaces found:', spacesData.results?.length || 0);

        if (!spacesData.results || spacesData.results.length === 0) {
            return {
                success: false,
                message: 'No Confluence spaces found. Please create a space first.'
            };
        }

        const spaceId = spacesData.results[0].id;
        const spaceKey = spacesData.results[0].key;
        console.log('Using space:', spaceKey, spaceId);

        // Build the page content with unique title (timestamp prevents duplicates)
        const timestamp = new Date().toISOString().slice(0, 19).replace('T', ' ');
        const pageTitle = `Sprint Retrospective: ${sprintName || 'Current Sprint'} - ${timestamp}`;

        const pageBody = `
<h2>🏎️ Sprint Retrospective: ${sprintName || 'Current Sprint'}</h2>
<p><em>Generated by SprintGPT on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}</em></p>

<h3>📊 Sprint Health Score</h3>
<ac:structured-macro ac:name="status">
<ac:parameter ac:name="title">${healthScore || 0}/100</ac:parameter>
<ac:parameter ac:name="colour">${(healthScore || 0) >= 70 ? 'Green' : (healthScore || 0) >= 50 ? 'Yellow' : 'Red'}</ac:parameter>
</ac:structured-macro>
<p>${summary || 'Sprint retrospective analysis.'}</p>
...
<td>${velocity || 0} story points</td>
</tr>
<tr>
<td><strong>Completion Rate</strong></td>
<td>${completionRate || 0}%</td>
</tr>
<tr>
<td><strong>Avg Cycle Time</strong></td>
<td>${avgCycleTime || 0} days</td>
</tr>
</tbody>
</table>
...
<h3>💬 Discussion Topics</h3>
<ol>
<li><strong>What went well this sprint?</strong></li>
<li><strong>What could we improve?</strong></li>
<li><strong>What actions should we take?</strong></li>
</ol>
...
<ac:task-body>Review and address any blockers from this sprint</ac:task-body>
</ac:task>
<ac:task>
<ac:task-status>incomplete</ac:task-status>
<ac:task-body>Document learnings and update team practices</ac:task-body>
</ac:task>
<ac:task>
<ac:task-status>incomplete</ac:task-status>
<ac:task-body>Schedule follow-up for action items</ac:task-body>
</ac:task>
</ac:task-list>

<hr/>
<p><em>🏎️ Powered by SprintGPT - AI Sprint Retrospective Engine</em></p>
`;

        // Create the page using v2 API
        const createResponse = await api.asUser().requestConfluence(
        ...
        console.log('Create page response status:', createResponse.status);
        ...
        if (createResponse.ok) {
            ...
            console.log('Create page failed:', createResponse.status, errorText);
            return {
                success: false,
                message: `Failed to create page: ${createResponse.status}`,
                error: errorText
            };
        }
